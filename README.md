...
```
pogo breaker <pokemon> [cp] [atk def hp] [league] [shadow] [fast] [fast:<move>] [dump]
```

For your Pokémon, against **each of the top 100 meta Pokémon in that league** (a
static list pulled from pvpoke's published overall rankings — `data/meta/<cp>.json`,
regenerated by `gen_meta.py`),
...
IVs give (or cost) you against each top-100 meta opponent. The classic
...
A 🛡 line shows the meta's best fast move hitting *you* (its bulkpoint) in the
same four columns.

The opponent is held **fixed at its default-IV stats** (re-derived from the
gamemaster `defaultIVs` table at full precision, pvpoke's ranking
`initialize(true)` path) plus shadow multipliers, so the only thing that
varies across your four columns is your own IVs.
...
- `fast` limits the report to fast moves (charged moves are noise at their
  damage floors); `fast:<move>` restricts to one specific move.
...
data/meta/<cp>.json  per-league top-100 meta (pvpoke overall rankings; regenerated by gen_meta.py)
gen_meta.py          rebuilds data/meta/ from pvpoke's rankings + gamemaster (top N, default 100)
